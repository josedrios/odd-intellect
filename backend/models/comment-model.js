...
export const getPostComments = (postId) =>
  query(
    "SELECT c.*, u.username, COUNT(r.id) AS reply_count FROM comments c JOIN users u ON c.user_id = u.id LEFT JOIN comments r ON c.id = r.parent_id WHERE c.post_id = $1 AND c.parent_id IS NULL GROUP BY c.id, u.username ORDER BY c.created_at DESC;",
    [postId],
  );
